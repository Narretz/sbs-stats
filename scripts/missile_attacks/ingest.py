...
# annotations. Replaces this file's hand-rolled _gh_notify — same two channels
# (annotation + job summary), now shared with every other ingest script.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from ingest_log import ann, get_logger  # noqa: E402

log = get_logger("missile-attacks")
...
def _add_missing_columns(conn, all_cols, col_type) -> list[str]:
    ...
    if added:
        ...
        log.warning(
            ...
            extra=ann(title="missile-attacks: piterfm added a CSV column"),
        )
    return added
