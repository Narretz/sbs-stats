...
- **Header growth is migrated, not fatal**: if piterfm *adds* a CSV column
  ...
  The run raises a WARNING naming the new column through the shared sink
  (`scripts/ingest_log.py`), which the workflow's `annotate_log.py` step turns
  into a GitHub annotation and a job-summary line — migrating a column is not
  the same as understanding it, and the run is the only moment anyone learns it
  appeared. (`status_data` was migrated silently and nobody looked until its
  placeholder 0s had been charted as real zeros for a week.) It's a warning,
  not a failure, so a purely additive change can't block the DB update and
  leave the data stale — but note that means it doesn't email anyone either, so
  it's only seen by whoever opens the run.
