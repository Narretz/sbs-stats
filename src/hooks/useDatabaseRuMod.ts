...
export function useDatabaseRuMod({ enabled = true }: { enabled?: boolean } = {}) {
  ...
  const queryDaily = useCallback(
    (days: number, endDate?: string): RuAdDailyRow[] => {
      ...
      // silent_days is intentionally NOT unioned in here: a day with no
      // standalone AD post isn't a "verified 0 drones" day — drones may
      // have been intercepted but only mentioned inside the Сводка stats
      // we don't yet parse, so an explicit 0 would be misleading. The
